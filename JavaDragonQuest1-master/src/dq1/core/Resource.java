...
//                                , areaOutTeleportPlayerDirection);
    ...
    public static void loadZones(String filePath) throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(Resource.class.getResourceAsStream(filePath)))) {
            String line;
            while ((line = reader.readLine()) != null) {
                ...
                String biome = args[2].replaceAll("\"", "").trim();
                String dungeon = args[3].replaceAll("\"", "").trim();
                int difficulty = Integer.parseInt(args[4].trim());
                int worldTier = Integer.parseInt(args[5].trim());
                double enemyScaling = Double.parseDouble(args[6].trim());
                double lootScaling = Double.parseDouble(args[7].trim());
                String description = args[8].replaceAll("\"", "").trim();
                ZONES.put(id, new Zone(id, name, biome, dungeon, difficulty, worldTier, enemyScaling, lootScaling, description));
            }
        }
    }

    public static Map<Integer, Zone> getZones() { return ZONES; }
